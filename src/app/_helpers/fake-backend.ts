...
export class FakeBackendInterceptor implements HttpInterceptor {
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    function authenticate() {
      ...
      if (!user) return error('MobileNo or password is incorrect');
      return ok({
        ...basicDetails(user),
    ...
    function register() {
      ...
      if (users.find(x => x.phone === user.phone)) {
        return error('MobileNo "' + user.phone + '" is already taken')
      }
    ...
    function basicDetails(user) {
      const { id, fullName, email, phone, Gender } = user;
      return { id, fullName, email, phone, Gender };
    }
